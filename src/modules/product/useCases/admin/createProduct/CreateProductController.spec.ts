...
describe("Create products", () => {
	...
	it("should not be able to create a new product with a measurement unit that not exists", async () => {
		const filePath = resolve(
			__dirname, 
			"..", 
			"..", 
		...
		expect(response.body.type).toEqual("measurementUnit.not.found");
		expect(response.body.field).toEqual("measurementUnitId");
	});

	it("should not be able to create a product with a bar code that contains letter", async () => {
		const measurementUnits = await prisma.measurementUnit.create({
			data: {
				name: "gramas",
				abbreviation: "g"
		...
		expect(response.body.type).toEqual("barCode.not.valid");
