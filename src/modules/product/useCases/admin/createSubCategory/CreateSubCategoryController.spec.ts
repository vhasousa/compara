...
describe("Create sub category", () => {
	...
	it("should not be able to create a new sub category with a name that already exists", async () => {
		...
		const response = await request(app).post("/sub_categories")
			.set("content-type", "multipart/form-data")
			.field("name", "Sub Category name")
			.field("description", "Sub Category description")
			.field("categoryName", category.body.name)
		...
		expect(response.body.type).toEqual("subCategory.already.exists");
		expect(response.body.field).toEqual("name");
	});

	it("should not be able to create a sub category without name", async () => {
		const filePath = resolve(
			__dirname, 
			"..", 
			"..", 
		...
		expect(subCategory.body.type).toEqual("name.required");
		expect(subCategory.body.field).toEqual("name");
	});
});